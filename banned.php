...
function show_list()
{
    global 	$lang_global, $lang_banned, $output, 
			$realm_db, 
			$itemperpage, $action_permission, $user_lvl;
    ...
    $result = $sqlr->query("
		SELECT $key_field, bandate, unbandate, bannedby, SUBSTRING_INDEX(banreason,' ',3) 
		FROM $fromwhere 
    ...
    while ($ban = $sqlr->fetch_row($result))
    {
        if ($ban_type === "account_banned")
        {
            ...
            $name_out = "<a href=\"accounts.php?action=edit_user&amp;error=11&amp;id=$ban[0]\">$owner_acc_name</a>";
        }
        else
        {
            $name_out = $ban[0];
