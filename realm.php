...
$output .= '
		...
		<th width="25%"><a href="realm.php?order_by=name&amp;dir='.$dir.'"'.($order_by==='name' ? ' class="'.$order_dir.'"' : '').'>'.$lang_realm['name'].'</a></th>
	...
	if($user_lvl >= $action_permission['delete'])
$output .= '
		<th width="35%"><a href="realm.php?order_by=address&amp;dir='.$dir.'"'.($order_by==='address' ? ' class="'.$order_dir.'"' : '').'>'.$lang_realm['address'].'</a></th>
		<th width="5%"><a href="realm.php?order_by=port&amp;dir='.$dir.'"'.($order_by==='port' ? ' class="'.$order_dir.'"' : '').'>'.$lang_realm['port'].'</a></th>';
$output .= '
		<th width="5%"><a href="realm.php?order_by=icon&amp;dir='.$dir.'"'.($order_by==='icon' ? ' class="'.$order_dir.'"' : '').'>'.$lang_realm['icon'].'</a></th>
		<th width="17%"><a href="realm.php?order_by=timezone&amp;dir='.$dir.'"'.($order_by==='timezone' ? ' class="'.$order_dir.'"' : '').'>'.$lang_realm['timezone'].'</a></th>
	</tr>';
unset($dir);
unset($order_dir);
unset($order_by);
	$flag_type = get_flag_type();
...
valid_login($action_permission['update']);
	...
	if 
	($realm = $sqlr->fetch_assoc($sqlr->query('
			SELECT realmlist.id AS rid, name, address, port, icon, timezone,
				(SELECT SUM(numchars) 
				FROM realmcharacters 
				WHERE realmid = rid) as sum
...
valid_login($action_permission['update']);
	...
	$query = $sqlr->query('
		UPDATE realmlist 
		SET name=\''.$new_name.'\', address =\''.$new_address.'\' , port =\''.$new_port.'\', icon =\''.$new_icon.'\', timezone =\''.$new_timezone.'\' 
		WHERE id = '.$id.'');
...
valid_login($action_permission['insert']);
	...
	if ($sqlr->query('
	INSERT INTO realmlist (id, name, address, port, icon, realmflags, timezone)
...
function get_flag_type()
{
	...
	return Array
	(
		0 => array(0,$lang_realm['none']),
		1 => array(1,$lang_realm['invalid']),
		2 => array(2,$lang_realm['offline']),
		4 => array(4,$lang_realm['specific_build']),
		8 => array(8,$lang_realm['unk1']),
		16 => array(16,$lang_realm['unk2']),
		32 => array(32,$lang_realm['new_players']),
		64 => array(64,$lang_realm['recommended']),
		128 => array(128,$lang_realm['full']),
	);
}
