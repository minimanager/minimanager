 ...
 while ($rows = $sql->fetch_row($result)) {
	...
	foreach($rows as $row => $value) {
		switch ($row) {
			...
			break;
			case 2:
			$value = "<a href=\"$item_datasite$rows[1]\" target=\"_blank\" onmouseover=\"toolTip,'item_tooltip')\"><img src=\"".get_icon($rows[1])."\" class=\"".get_item_border($rows[1])."\" alt=\"$value\"><br/>$value".(($rows[8]>1) ? " (x$rows[8])" : "")."</img></a>";
			break;
			case 0:
 ...
 while ($rows = $sql->fetch_row($result)) {
	...
	foreach($rows as $row => $value) {
		switch ($row) {
			...
			break;
			case 2:
			$value = "<a href=\"$item_datasite$rows[1]\" target=\"_blank\" onmouseover=\"toolTip,'item_tooltip')\"><img src=\"".get_icon($rows[1])."\" class=\"".get_item_border($item[0])."\" alt=\"$value\"><br/>$value".(($rows[8]>1) ? " (x$rows[8])" : "")."</img></a>";
			break;
			case 0:
			$value = "<b>".((!empty($rows[9])) ? "<font color=".$sidecolor[$rows[9]].">".htmlentities($value)."</font>" : "N/A")."</b>";
			break;
