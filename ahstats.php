...
function browse_auctions()
{
  ...
  $this_page = $sqlc->num_rows($result);
              ...
              <td width=\"80%\">
                <form action=\"ahstats.php\" method=\"get\" name=\"form\">
                  ...
                  <table class=\"hidden\">
                    <tr>
                      <td>
                        <input type=\"text\" size=\"25\" name=\"search_value\" />
                      </td>
                      <td>
...
function search_auctions()
{
 ...
 switch ($search_by) {
  ...
  case "item_id":
    $search_filter = "
          AND auctionhouse.item_template = '$search_value'";
    break;
...
$result = $sql->query("SELECT `characters`.`name` AS `seller`, `auctionhouse`.`item_template` AS `itemid`, `item_template`.`name` AS `itemname`, `auctionhouse`.`buyoutprice` AS `buyout`,
  ...
  foreach($rows as $row => $value)
  {
    switch ($row)
    {
      ...
      case 3:
        ...
        $c = $value;
        $value = $g."
          <img src=\"./img/gold.gif\" /> ".$s."<img src=\"./img/silver.gif\" /> ".$c."<img src=\"./img/copper.gif\" /> ";
      break;

      case 2:
        $value = "<a href=\"$item_datasite$rows[1]\" target=\"_blank\" onmouseover=\"toolTip,'item_tooltip')\"><img src=\"".get_item_icon($rows[1])."\" class=\"".get_item_border($item[0])."\" alt=\"$value\"><br/>$value".(($rows[8]>1) ? " (x$rows[8])" : "")."</img></a>";
      break;
 ...
 $output .= "
         <tr>
           <td colspan=\"7\" class=\"hidden\" align=\"right\">{$lang_auctionhouse['tot_found']} : $tot_found {$lang_global['limit']} : $sql_search_limit
           </td>
         </tr>
   </table>
   </center>";
