...
//==========================MyGuild==========================================
	...
	$query_myGuild = $sqlc->query("
		SELECT g.guildid AS gid, g.name, g.leaderguid AS lguid,
			(SELECT name FROM characters WHERE guid = lguid),
			(SELECT race in (2,5,6,8,10) FROM characters WHERE guid = lguid) AS faction,
			(SELECT count(*) FROM characters WHERE guid in
				(SELECT guid FROM guild_member WHERE guildid = lguid)
			and online = 1) AS gonline,
			(SELECT count(*) FROM guild_member WHERE guildid = gid),
			SUBSTRING_INDEX(g.MOTD,' ',6), g.createdate,
			(SELECT account FROM characters WHERE guid = lguid)
		FROM guild AS g
			LEFT OUTER JOIN guild_member AS gm on gm.guildid = g.guildid
			LEFT OUTER JOIN characters AS c on c.guid = gm.guid
		WHERE c.account = $user_id group by g.guildid
		ORDER BY gid");
...
$output .= 
			($user_lvl < $owner_gmlvl ) ? "<td>".htmlentities($data[3])."</td>" : "<td><a href=\"char.php?id=$data[2]&amp;realm=$realmid\">".htmlentities($data[3])."</a></td>";
$output .= "
		...
		switch($search_by)
		{
			case "name":
				...
				$query = $sqlc->query("
					SELECT g.guildid AS gid, g.name,g.leaderguid AS lguid,
						...
						c.race in (2,5,6,8,10) AS lfaction,
						(SELECT count(*) FROM guild_member WHERE guildid = gid) AS tot_chars,
						createdate, c.account AS laccount
					FROM guild AS g
						LEFT OUTER JOIN characters AS c on c.guid = g.leaderguid
					WHERE g.name like '%$search_value%'
					ORDER BY $order_by $order_dir
					LIMIT $start, $itemperpage");
				$query_count = $sqlc->query("
					SELECT 1
					FROM guild
			...
			case "guildid" :
				...
				$query = $sqlc->query("
				  ...
				  FROM guild AS g
					LEFT OUTER JOIN characters AS c on c.guid = g.leaderguid
				  WHERE g.guildid = '$search_value'
				  ORDER BY $order_by $order_dir
				  LIMIT $start, $itemperpage");
				$query_count = $sqlc->query("
					SELECT 1
					FROM guild
					WHERE guildid = '$search_value'");
			break;
			default :
...
$output .=
					generate_pagination("guild.php?action=brows_guilds&amp;realm=$realmid&amp;order_by=$order_by&amp;".($search_value && $search_by ? "search_by=$search_by&amp;search_value=$search_value&amp" : "")."dir=".(($dir) ? 0 : 1)."", $all_record, $itemperpage, $start);
$output .= "
				</td>
			</tr>
			<tr>
...
$output .= "
			<tr>
				<td>$data[0]</td>";
$output .= 
				($user_lvl >= $action_permission['update']) ? "<td><a href=\"guild.php?action=view_guild&amp;realm=$realmid&amp;error=3&amp;id=$data[0]\">".htmlentities($data[1])."</a></td>" : "<td>".htmlentities($data[1])."</td>";
$output .=
				($user_lvl < $owner_gmlvl ) ? "<td>".htmlentities($data[3])."</td>" : "<td><a href=\"char.php?id=$data[2]&amp;realm=$realmid\">".htmlentities($data[3])."</a></td>";
$output .= "
				<td><img src=\"img/".($data[4]==0 ? "alliance" : "horde")."_small.gif\" alt=\"\" /></td>
...
function view_guild()
{
  ...
  $q_inguild = $sqlc->query("
	SELECT 1
	FROM guild_member
	WHERE guildid = '$guild_id' and guid in
		(SELECT guid
		FROM characters
  ...
  $q_amIguildleader = $sqlc->query("
	SELECT 1
	FROM guild
	WHERE guildid = '$guild_id' and leaderguid in
		(SELECT guid
		FROM characters
		WHERE account = '$user_id')");
  $amIguildleader = $sqlc->result($q_amIguildleader, 0, '1');

  $q_guildmemberCount = $sqlc->query("
	SELECT 1
	FROM guild_member
  ...
  $query = $sqlc->query("
	SELECT guildid, name, info, MOTD, createdate,
		(SELECT count(*)
		FROM guild_member
		WHERE guildid = '$guild_id') AS mtotal,
		(SELECT count(*)
		FROM guild_member
		WHERE guildid = '$guild_id' and guid in
		(SELECT guid
		FROM characters
		WHERE online = 1)) AS monline
    FROM guild
	WHERE guildid = '$guild_id'");
...
";
  ...
  $members = $sqlc->query("
	SELECT gm.guid AS cguid, c.name AS cname, c.`race` AS crace ,c.`class` AS cclass, c.`level` AS clevel, gm.rank AS mrank,
		(SELECT rname
		FROM guild_rank
		WHERE guildid ='$guild_id' AND rid = mrank) AS rname,
