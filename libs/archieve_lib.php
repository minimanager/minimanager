...
{
	$achievement_name = $sqlm->fetch_assoc($sqlm->query('
		SELECT name01 
...
{
	$achievement_reward = $sqlm->fetch_assoc($sqlm->query('
		SELECT rewarddesc01 
		FROM dbc_achievement 
		WHERE id ='.$id.' LIMIT 1'));
...
{
	$achievement_points = $sqlm->fetch_assoc($sqlm->query('
		SELECT rewpoints
		FROM dbc_achievement
		WHERE id = '.$id.' LIMIT 1'));
...
{
	$category_id= $sqlm->fetch_assoc($sqlm->query('
		SELECT categoryid 
		FROM dbc_achievement 
		WHERE id = '.$id.' LIMIT 1'));
	$category_name = $sqlm->fetch_assoc($sqlm->query('
		SELECT name01
		FROM dbc_achievement_category
...
function achieve_get_id_category($id, &$sqlm)
{
	$achieve_cat = array();
	$result = ($sqlm->query('
		SELECT id, name01, description01, rewarddesc01, rewpoints
		FROM dbc_achievement
		WHERE categoryid = \''.$id.'\'
		ORDER BY `order` DESC'));
...
function achieve_get_icon($achieveid, &$sqlm=0)
{
	global	$mmfpm_db,
			$proxy_cfg,
