...
function get_item_icon($itemid)
{
  ...
  $result = $sql_3->query("SELECT `displayid` FROM `item_template` WHERE `entry` = $itemid LIMIT 1");
  ...
  if ($displayid)
  {
    $result = $sql_3->query("SELECT `name` FROM `dbc_itemdisplayinfo` WHERE `id`=$displayid LIMIT 1");
...
function get_spell_icon($auraid)
{
  ...
  $result = $sql_4->query("SELECT `spellicon` FROM `dbc_spell` WHERE `spellID`=$auraid LIMIT 1");

  if ($result)
    $displayid = $sql_4->result($result, 0);
  else
    $displayid = 0;

  if ($displayid)
  {
    $result = $sql_4->query("SELECT `name` FROM `dbc_spellicon` WHERE `id` = $displayid LIMIT 1");
